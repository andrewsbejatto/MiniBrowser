...
var
  FrmMiniBrowser : TFrmMiniBrowser;
...
procedure TFrmMiniBrowser.BackBtnClick(Sender: TObject);
begin
  Chromium1.GoBack;
end;

procedure TFrmMiniBrowser.ForwardBtnClick(Sender: TObject);
begin
  Chromium1.GoForward;
end;

procedure TFrmMiniBrowser.GoBtnClick(Sender: TObject);
begin
  Chromium1.LoadURL(URLCbx.Text);
end;

procedure TFrmMiniBrowser.ReloadBtnClick(Sender: TObject);
begin
  Chromium1.Reload;
end;

procedure TFrmMiniBrowser.Resetzoom1Click(Sender: TObject);
begin
  Chromium1.ResetZoomStep;
end;

procedure TFrmMiniBrowser.Resolvehost1Click(Sender: TObject);
var
  TempURL : string;
begin
...
procedure TFrmMiniBrowser.CEFinfo1Click(Sender: TObject);
var
  TempInfo : string;
begin
...
procedure TFrmMiniBrowser.Chromium1AddressChange(Sender: TObject;
  const browser: ICefBrowser; const frame: ICefFrame; const url: ustring);
begin
  if Chromium1.IsSameBrowser(browser) then AddURL(url);
end;

procedure TFrmMiniBrowser.Chromium1AfterCreated(Sender: TObject; const browser: ICefBrowser);
begin
...
procedure TFrmMiniBrowser.Chromium1BeforeClose(Sender: TObject; const browser: ICefBrowser);
begin
...
procedure TFrmMiniBrowser.Chromium1BeforeContextMenu(Sender: TObject;
...
procedure TFrmMiniBrowser.Chromium1BeforeDownload(Sender: TObject;
...
procedure TFrmMiniBrowser.Chromium1BeforeResourceLoad(Sender: TObject;
  const browser: ICefBrowser; const frame: ICefFrame;
...
procedure TFrmMiniBrowser.Chromium1CanDownload(Sender: TObject;
...
procedure TFrmMiniBrowser.Chromium1ConsoleMessage(Sender: TObject;
...
procedure TFrmMiniBrowser.Chromium1ContextMenuCommand(Sender: TObject;
  const browser: ICefBrowser; const frame: ICefFrame;
...
procedure TFrmMiniBrowser.Chromium1CookiesFlushed(Sender: TObject);
begin
  PostMessage(Handle, MINIBROWSER_COOKIESFLUSHED, 0, 0);
end;

procedure TFrmMiniBrowser.Chromium1CursorChange(Sender: TObject;
...
procedure TFrmMiniBrowser.CookiesFlushedMsg(var aMessage : TMessage);
begin
  showmessage('The cookies were flushed successfully');
end;

procedure TFrmMiniBrowser.PrintPDFEndMsg(var aMessage : TMessage);
begin
...
procedure TFrmMiniBrowser.PreferencesAvailableMsg(var aMessage : TMessage);
begin
  if (aMessage.lParam <> 0) then
...
procedure TFrmMiniBrowser.Chromium1DownloadUpdated(Sender: TObject;
  const browser: ICefBrowser; const downloadItem: ICefDownloadItem;
...
procedure TFrmMiniBrowser.Chromium1FileDialog(      Sender                 : TObject;
...
function TFrmMiniBrowser.ShowOpenFileDialog(var aFilePaths : TStringList; aMultiple : boolean) : boolean;
var
...
function TFrmMiniBrowser.ShowOpenFolderDialog(var aFilePaths : TStringList) : boolean;
var
...
function TFrmMiniBrowser.ShowSaveFileDialog(var aFilePaths : TStringList) : boolean;
var
...
procedure TFrmMiniBrowser.ShowFileDialogMsg(var aMessage : TMessage);
var
  TempResult    : boolean;
...
procedure TFrmMiniBrowser.SelectCertificateMsg(var aMessage : TMessage);
var
...
procedure TFrmMiniBrowser.Chromium1FullScreenModeChange(Sender: TObject;
...
procedure TFrmMiniBrowser.Chromium1GetAuthCredentials(Sender: TObject; const browser: ICefBrowser; const originUrl: ustring; isProxy: Boolean;
  const host: ustring; port: Integer; const realm, scheme: ustring; const callback: ICefAuthCallback; out Result: Boolean);
begin
  callback.Cont(FAuthenticationUsername, FAuthenticationPassword);
end;

procedure TFrmMiniBrowser.Chromium1KeyEvent(Sender: TObject;
...
procedure TFrmMiniBrowser.Allowdownloads1Click(Sender: TObject);
begin
  FAllowDownloads := not(FAllowDownloads);
end;

procedure TFrmMiniBrowser.ApplicationEvents1Message(var Msg: tagMSG;
  var Handled: Boolean);
begin
  case Msg.message of
...
procedure TFrmMiniBrowser.HandleKeyUp(const aMsg : TMsg; var aHandled : boolean);
var
...
procedure TFrmMiniBrowser.HandleKeyDown(const aMsg : TMsg; var aHandled : boolean);
var
  TempMessage : TMessage;
  TempKeyMsg  : TWMKey;
begin
...
procedure TFrmMiniBrowser.Chromium1LoadEnd(Sender: TObject;
  const browser: ICefBrowser; const frame: ICefFrame;
...
procedure TFrmMiniBrowser.Chromium1LoadingProgressChange(Sender: TObject;
  const browser: ICefBrowser; const progress: Double);
begin
  StatusBar1.Panels[0].Text := 'Loading... ' + FloatToStrF(progress * 100, ffFixed, 3, 0) + '%';
end;

procedure TFrmMiniBrowser.Chromium1LoadingStateChange(Sender: TObject;
...
procedure TFrmMiniBrowser.Chromium1MediaAccessChange(Sender: TObject;
...
procedure TFrmMiniBrowser.Chromium1NavigationVisitorResultAvailable(Sender: TObject;
...
procedure TFrmMiniBrowser.Chromium1PdfPrintFinished(Sender: TObject; aResultOK: Boolean);
begin
  PostMessage(Handle, MINIBROWSER_PDFPRINT_END, 0, LPARAM(aResultOK));
end;

procedure TFrmMiniBrowser.Chromium1PrefsAvailable(Sender: TObject; aResultOK: Boolean);
begin
  PostMessage(Handle, MINIBROWSER_PREFS_AVLBL, 0, LPARAM(aResultOK));
end;

procedure TFrmMiniBrowser.Chromium1PreKeyEvent(Sender: TObject;
  const browser: ICefBrowser; const event: PCefKeyEvent; osEvent: TCefEventHandle;
...
procedure TFrmMiniBrowser.Chromium1RenderCompMsg(Sender: TObject; var aMessage : TMessage; var aHandled: Boolean);
begin
...
procedure TFrmMiniBrowser.Chromium1RequestMediaAccessPermission(Sender: TObject;
  const browser: ICefBrowser; const frame: ICefFrame;
...
procedure TFrmMiniBrowser.MediaAccessRequestMsg(var aMessage : TMessage);
var
  TempMessage : string;
...
procedure TFrmMiniBrowser.Chromium1ResolvedHostAvailable(Sender: TObject;
...
procedure TFrmMiniBrowser.InspectRequest(const aRequest : ICefRequest);
var
...
procedure TFrmMiniBrowser.InspectResponse(const aResponse : ICefResponse);
var
  TempHeaderMap : ICefStringMultimap;
  i, j : integer;
begin
...
procedure TFrmMiniBrowser.LoadPreferences;
begin
...
//    Chromium1.ProxyType              := PreferencesFrm.ProxyTypeCbx.ItemIndex;
    Chromium1.ProxyServer            := LIni.ReadString('Preferences', 'ProxyServerEdt', '');
    Chromium1.ProxyPort              := LIni.ReadInteger('Preferences', 'PreferencesFrm.ProxyPortEdt', 80);
    Chromium1.ProxyUsername          := LIni.ReadString('Preferences', 'ProxyUsernameEdt', '');
    Chromium1.ProxyPassword          := LIni.ReadString('Preferences', 'ProxyPasswordEdt', '');
    Chromium1.ProxyScriptURL         := LIni.ReadString('Preferences', 'ProxyScriptURLEdt', '');
    Chromium1.ProxyByPassList        := LIni.ReadString('Preferences', 'ProxyByPassListEdt', '');
    Chromium1.CustomHeaderName       := LIni.ReadString('Preferences', 'HeaderNameEdt', '');
    Chromium1.CustomHeaderValue      := LIni.ReadString('Preferences', 'HeaderValueEdt', '');
    Chromium1.MaxConnectionsPerProxy := LIni.ReadInteger('Preferences', 'MaxConnectionsPerProxyEdt', 0);
    FAuthenticationUsername          := LIni.ReadString('Preferences', 'AuthenticationUsernameEdt', '');
    FAuthenticationPassword          := LIni.ReadString('Preferences', 'AuthenticationPasswordEdt', '');

    var a := LIni.ReadString('Preferences', 'URL', '');
    if a <> EmptyStr then
      URLCbx.Items.CommaText := a;
  finally
    LIni.Free;
  end;
...
procedure TFrmMiniBrowser.Memoryinfo1Click(Sender: TObject);
const
...
procedure TFrmMiniBrowser.Toggleaudio1Click(Sender: TObject);
begin
  Chromium1.ToggleAudioMuted;
end;

procedure TFrmMiniBrowser.Chromium1ResourceResponse(Sender: TObject;
  const browser: ICefBrowser; const frame: ICefFrame;
...
procedure TFrmMiniBrowser.ShowStatusText(const aText : string);
begin
  if not(FClosing) then StatusBar1.Panels[1].Text := aText;
end;

procedure TFrmMiniBrowser.Simulatekeyboardpresses1Click(Sender: TObject);
const
...
procedure TFrmMiniBrowser.StopBtnClick(Sender: TObject);
begin
  Chromium1.StopLoad;
end;

procedure TFrmMiniBrowser.Chromium1SelectClientCertificate(Sender: TObject;
...
procedure TFrmMiniBrowser.Chromium1StatusMessage(Sender: TObject;
  const browser: ICefBrowser; const value: ustring);
begin
  if Chromium1.IsSameBrowser(browser) then ShowStatusText(value);
end;

procedure TFrmMiniBrowser.Chromium1TextResultAvailable(Sender: TObject; const aText: ustring);
begin
  clipboard.AsText := aText;
end;

procedure TFrmMiniBrowser.Chromium1TitleChange(Sender: TObject;
...
procedure TFrmMiniBrowser.Chromium1ZoomPctAvailable(Sender: TObject;
  const aZoomPct: Double);
begin
  ShowStatusText('Zoom : ' + floattostr(aZoomPct) + '%');
end;

procedure TFrmMiniBrowser.ClearallstorageforcurrentURL1Click(Sender: TObject);
begin
  Chromium1.ClearDataForOrigin(URLCbx.Text);
end;

procedure TFrmMiniBrowser.Clearcache1Click(Sender: TObject);
begin
  Chromium1.ClearCache;
end;

procedure TFrmMiniBrowser.FindText1Click(Sender: TObject);
begin
  FindFrm.Show;
end;

procedure TFrmMiniBrowser.Flushcookies1Click(Sender: TObject);
begin
  if not(Chromium1.FlushCookieStore(False)) then
    showmessage('There was a problem flushing the cookies.');
end;

procedure TFrmMiniBrowser.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  CanClose := FCanClose;
...
procedure TFrmMiniBrowser.FormCreate(Sender: TObject);
begin
  FCanClose            := False;
  FClosing             := False;
...
procedure TFrmMiniBrowser.FormDestroy(Sender: TObject);
begin
  var LIni := TIniFile.Create(TPath.Combine(TDirectory.GetCurrentDirectory, 'browser.ini'));
  try
    LIni.WriteString('Preferences', 'URL', URLCbx.Items.DelimitedText);
  finally
    LIni.Free;
  end;
...
procedure TFrmMiniBrowser.FormShow(Sender: TObject);
begin
...
procedure TFrmMiniBrowser.Timer1Timer(Sender: TObject);
begin
...
procedure TFrmMiniBrowser.URLCbxKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  if key = VK_RETURN then
    GoBtnClick(Sender);
end;

procedure TFrmMiniBrowser.Useragent1Click(Sender: TObject);
var
...
procedure TFrmMiniBrowser.BrowserCreatedMsg(var aMessage : TMessage);
begin
  CEFWindowParent1.UpdateSize;
  NavControlPnl.Enabled := True;
end;

procedure TFrmMiniBrowser.BrowserDestroyMsg(var aMessage : TMessage);
begin
  FreeAndNil(CEFWindowParent1);
end;

procedure TFrmMiniBrowser.btnBookmarksClick(Sender: TObject);
begin
  if not Assigned(frmLinks) then
    Application.CreateForm(TfrmLinks, frmLinks);
  frmLinks.ProcedureOpenUrl := OpenURL;
  frmLinks.Show;
end;

procedure TFrmMiniBrowser.Acceptlanguage1Click(Sender: TObject);
var
...
procedure TFrmMiniBrowser.AddURL(const aURL : string);
begin
...
procedure TFrmMiniBrowser.DestroyCertificates;
var
  i : integer;
begin
...
procedure TFrmMiniBrowser.akescreenshot1Click(Sender: TObject);
begin
  FPendingMsgID := DEVTOOLS_SCREENSHOT_MSGID;
  Chromium1.ExecuteDevToolsMethod(0, 'Page.captureScreenshot', nil);
end;

procedure TFrmMiniBrowser.Chromium1DevToolsMethodResult(      Sender     : TObject;
                                                        const browser    : ICefBrowser;
                                                              message_id : Integer;
                                                              success    : Boolean;
...
procedure TFrmMiniBrowser.DevToolsDataAvailableMsg(var aMessage : TMessage);
var
  TempData : TBytes;
...
procedure TFrmMiniBrowser.ShowDevToolsMsg(var aMessage : TMessage);
var
  TempPoint : TPoint;
begin
...
procedure TFrmMiniBrowser.HideDevToolsMsg(var aMessage : TMessage);
begin
  HideDevTools;
  Chromium1.SetFocus(True);
end;

procedure TFrmMiniBrowser.Inczoom1Click(Sender: TObject);
begin
  Chromium1.IncZoomStep;
end;

procedure TFrmMiniBrowser.Openfile1Click(Sender: TObject);
begin
...
procedure TFrmMiniBrowser.OpenfilewithaDAT1Click(Sender: TObject);
var
...
procedure TFrmMiniBrowser.OpenURL(AUrl: String);
begin
  Chromium1.LoadURL(AUrl);
end;

procedure TFrmMiniBrowser.PopupMenu1Popup(Sender: TObject);
begin
  if DevTools.Visible then
...
procedure TFrmMiniBrowser.Preferences1Click(Sender: TObject);
begin
  ...
  PreferencesFrm.AuthenticationUsernameEdt.Text   := FAuthenticationUsername;
  PreferencesFrm.AuthenticationPasswordEdt.Text   := FAuthenticationPassword;
  ...
  if (PreferencesFrm.ShowModal = mrOk) then
    begin
      ...
      FAuthenticationUsername          := PreferencesFrm.AuthenticationUsernameEdt.Text;
      FAuthenticationPassword          := PreferencesFrm.AuthenticationPasswordEdt.Text;
...
procedure TFrmMiniBrowser.Print1Click(Sender: TObject);
begin
  Chromium1.Print;
end;

procedure TFrmMiniBrowser.PrintinPDF1Click(Sender: TObject);
begin
...
procedure TFrmMiniBrowser.ConfigBtnClick(Sender: TObject);
var
  TempPoint : TPoint;
begin
...
procedure TFrmMiniBrowser.CopyHTMLMsg(var aMessage : TMessage);
begin
  Chromium1.RetrieveHTML;
end;

procedure TFrmMiniBrowser.CopyAllTextMsg(var aMessage : TMessage);
var
  TempName : string;
begin
...
procedure TFrmMiniBrowser.CopyFramesIDsMsg(var aMessage : TMessage);
var
  i          : NativeUInt;
...
procedure TFrmMiniBrowser.CopyFramesNamesMsg(var aMessage : TMessage);
var
  TempSL : TStringList;
begin
  try
...
procedure TFrmMiniBrowser.ShowResponseMsg(var aMessage : TMessage);
begin
...
procedure TFrmMiniBrowser.ShowNavigationMsg(var aMessage : TMessage);
begin
  SimpleTextViewerFrm.Memo1.Lines.Clear;
...
procedure TFrmMiniBrowser.SaveasMHTML1Click(Sender: TObject);
var
...
procedure TFrmMiniBrowser.SavePreferencesMsg(var aMessage : TMessage);
begin
...
procedure TFrmMiniBrowser.TakeSnapshotMsg(var aMessage : TMessage);
var
  TempBitmap : TBitmap;
begin
  TempBitmap := nil;
...
procedure TFrmMiniBrowser.WMMove(var aMessage : TWMMove);
begin
  inherited;
...
procedure TFrmMiniBrowser.WMMoving(var aMessage : TMessage);
begin
  inherited;
...
procedure TFrmMiniBrowser.WMEnterMenuLoop(var aMessage: TMessage);
begin
  inherited;
...
procedure TFrmMiniBrowser.WMExitMenuLoop(var aMessage: TMessage);
begin
  inherited;
...
procedure TFrmMiniBrowser.WMQueryEndSession(var aMessage: TWMQueryEndSession);
begin
...
procedure TFrmMiniBrowser.Deczoom1Click(Sender: TObject);
begin
  Chromium1.DecZoomStep;
end;

procedure TFrmMiniBrowser.DevTools1Click(Sender: TObject);
begin
  if DevTools.Visible then
    HideDevTools
   else
...
procedure TFrmMiniBrowser.Downloadimage1Click(Sender: TObject);
var
  TempURL : string;
begin
...
procedure TFrmMiniBrowser.Chromium1DownloadImageFinished(      Sender         : TObject;
...
procedure TFrmMiniBrowser.ShowDevTools(aPoint : TPoint);
begin
...
procedure TFrmMiniBrowser.ShowDevTools;
var
  TempPoint : TPoint;
begin
...
procedure TFrmMiniBrowser.HideDevTools;
begin
